...
// helper: get MONGO_URI from env
const MONGO_URI = process.env.MONGO_URI || "";

// simple DB ping route to verify connection
app.get("/api/ping-db", async (req, res) => {
  if (!MONGO_URI) {
    return res.status(400).json({ ok: false, msg: "MONGO_URI not set" });
  }
  try {
...
// videos API (simple, uses mongo with 'videos' collection)
...
// add video (admin) - NOTE: frontend demo only, secure later
app.post("/api/videos", async (req, res) => {
  const secret = req.body?.secret || req.headers["x-admin-secret"];
  // optional check — set ADMIN_SECRET env on Render for safety
  ...
  const vid = (function extractId(u){
    try {
